...
export default function Card(props) {
    return (
        <div className = 'card'>
            ...
            <img width = '170px' height = '230px' src= {props.img} alt='of Katie Zaferes'/>
            <div className= 'onlineStatus'>SOLD OUT</div>
            </div>
            <div className = 'cardInfo'>
                <p className = 'cardInfoFirstLine'> {star} 
                {props.rating} <span className = 'locationText'> ({props.reviewCount}) <span className = 'dot'>• </span> {props.country} </span>
                </p>
                <p>
                {props.title}
                </p>
